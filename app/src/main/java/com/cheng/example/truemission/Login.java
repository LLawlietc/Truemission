...
import com.users.usersactivity.User;
...
public class Login extends Activity {                 //登录界面活动
    ...
    private Button mprofessorButton;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mprofessorButton=(Button)findViewById(R.id.login_btn_professor);
        ...
        mprofessorButton.setOnClickListener(mListener);
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.login_text_change_pwd:                             //登录界面的修改密码按钮
                    ...
                    finish();
                    break;
                case R.id.login_btn_professor:
                    Intent intent_Login_to_professor = new Intent(Login.this,Professor_Login.class) ;    //切换Login Activity至Professor_Login Activity
                    startActivity(intent_Login_to_professor);
                    finish();
                    break;
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            String userPwd = mPwd.getText().toString().trim();//trim()返回调用字符串对象的一个副本
    ...
    public void cancel() {           //注销
        if (isUserNameAndPwdValid()) {
            String userName = mAccount.getText().toString().trim();    //获取当前输入的用户名和密码信息
            String userPwd = mPwd.getText().toString().trim();//trim()去掉前后空格
            int result=mUserDataManager.findUserByNameAndPwd(userName, userPwd);
            if(result==1){                                             //返回1说明用户名和密码均正确
//                Intent intent = new Intent(Login.this,User.class) ;    //切换Login Activity至User Activity
//                startActivity(intent);
